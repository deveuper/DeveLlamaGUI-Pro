...
| GPU layers | `--n-gpu-layers` | Layers to offload to GPU |
...
| KV cache type | `--cache-type-k/v` | KV cache quantization format |
...
- ⚙️ **全パラメータ対応** — GPUレイヤー、コンテキストサイズ、バッチサイズ、Flash Attention、KVキャッシュタイプ、連続バッチ処理など
...
| GPUレイヤー | `--n-gpu-layers` | GPUにオフロードするレイヤー数 |
...
| KVキャッシュタイプ | `--cache-type-k/v` | KVキャッシュ量子化フォーマット |
...
- ⚙️ **전체 매개변수 지원** — GPU 레이어, 컨텍스트 크기, 배치 크기, Flash Attention, KV 캐시 타입, 연속 배치 처리 등
...
| GPU 레이어 | `--n-gpu-layers` | GPU에 오프로드할 레이어 수 |
...
| KV 캐시 타입 | `--cache-type-k/v` | KV 캐시 양자화 포맷 |
